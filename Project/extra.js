...
router.post('/pitches', async(req,res) => {
    ...
    if(req.body.constructor === Object && Object.keys(req.body).length === 0 || isValidKey({investor, amount, equity, comment})) {
        res.status(400).json({});
        return;
    }
    try{
...
router.post('/pitches/:pitchId/makeOffer', async(req,res) => {
    ...
    Pitch.findByIdAndUpdate(
        ...
        (err,data) => {
            ...
            if(data == null)res.status(404).send({message: "Pitch Not Found"});
